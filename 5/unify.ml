...
let rec unify (t1: typ) (t2: typ) : substitution =
	...
	|TUVar x, TUVar y when (x = y) -> []
  	|TUVar x, _ -> if occurs x t2 then 
				raise (UnificationFailure "Can't Unify: TUVAR X") else [(x, t2)]
  	|_, TUVar y -> if occurs y t1 then 
				raise (UnificationFailure "Can't Unify: TUVAR Y") else [(y, t1)]
  	|TList x, TList y -> unify x y
  	|TArrow (x, y), TArrow(w, z) -> let o = unify x w 
						in  (unify (sub_all o y) (sub_all o z)) 
  	|TProd (x, y), TProd (w, z) -> let o = unify x w 
						in (unify (sub_all o y) (sub_all o z)) 
  	|_ -> raise (UnificationFailure "Can't Unify")
